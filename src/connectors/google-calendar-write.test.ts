...
import { createGoogleCalendarWriteOperations, GOOGLE_CALENDAR_EVENT_CREATE_ACTION, GOOGLE_CALENDAR_EVENT_DELETE_ACTION, GOOGLE_CALENDAR_EVENT_UPDATE_ACTION, validateGoogleCalendarEventUpdateInput } from "./google-calendar-write.js";
...
describe("Google Calendar gog write connector", () => {
  ...
  it("suppresses undisclosed attendee notifications for gog update and delete", async () => {
    const root = await mkdtemp(join(tmpdir(), "calendar-gog-notify-"));
    const executablePath = await createFakeGog(root);
    const fetcher = vi.fn(async () => new Response(JSON.stringify({ access_token: "access-token-canary-value", token_type: "Bearer" }), { status: 200 }));
    const operations = createGoogleCalendarWriteOperations({ backend: "gog", gog: { executablePath, executableSha256: await fileSha256(executablePath), configRoot: root, fetch: fetcher as typeof fetch } });
    ...
    const context = { claims: {} as never, material: { kind: "oauth2" as const, refreshToken: "refresh-canary", clientId: "client-id" }, assertCredentialActive: async () => {}, markProviderCallStarted: () => {} };
    await update.execute(context, { eventId: "event_12345", location: "Room 2" });
    let invocation = JSON.parse(await readFile(join(root, "fake-gog-invocation.json"), "utf8")) as { argv: string[] };
    expect(invocation.argv).toContain("none"); expect(invocation.argv).not.toContain("all");
    await remove.execute(context, { eventId: "event_12345" });
