...
describe("Google Calendar gog write connector", () => {
  it("validates updates and invokes the exact baked gog create command", async () => {
    ...
    const operation = createGoogleCalendarWriteOperations({ backend: "gog", gog: { executablePath, configRoot: root, fetch: fetcher as typeof fetch } }).find((item) => item.action === GOOGLE_CALENDAR_EVENT_CREATE_ACTION)!;
