...
export function canonicalGoogleOAuthScope(scope: string): string {
  if (scope === "email") return GOOGLE_USERINFO_EMAIL_SCOPE;
  if (scope === "profile") return GOOGLE_USERINFO_PROFILE_SCOPE;
  return scope;
}
...
async function validateGoogleIdToken(idToken: string, audience: string, fetcher: Fetch): Promise<{ issuer: string; audience: string; nonce: string }> {
  ...
  if (parts.length !== 3 || parts.some((part) => part.length === 0)) throw new Error("GOOGLE_OAUTH_ID_TOKEN_FORMAT_INVALID");
  const header = decodeJwtPart(parts[0]!); const claims = decodeJwtPart(parts[1]!);
  if (header.alg !== "RS256" || typeof header.kid !== "string") throw new Error("GOOGLE_OAUTH_ID_TOKEN_HEADER_INVALID");
  const jwksResponse = await fetcher(GOOGLE_JWKS_ENDPOINT, { method: "GET", redirect: "error", signal: AbortSignal.timeout(10_000) });
  if (!jwksResponse.ok) throw new Error("GOOGLE_OAUTH_JWKS_HTTP_ERROR");
  const jwks = await boundedJson(jwksResponse, 128 * 1024);
  if (!Array.isArray(jwks.keys)) throw new Error("GOOGLE_OAUTH_JWKS_INVALID");
  ...
  if (!jwk) throw new Error("GOOGLE_OAUTH_ID_TOKEN_KEY_UNKNOWN");
  let valid = false;
  ...
  if (!valid || claims.iss !== "https://accounts.google.com" && claims.iss !== "accounts.google.com" || !tokenAudience.includes(audience) ||
    ...
    typeof claims.iat !== "number" || claims.iat > now + 60) throw new Error("GOOGLE_OAUTH_ID_TOKEN_CLAIMS_INVALID");
...
export class GoogleOAuthTokenExchanger implements OAuthTokenExchanger {
  ...
  async exchange(input: { authorizationCode: string; pkceVerifier: string; redirectTargetId: string }): Promise<OAuthTokenExchangeResult> {
    ...
    if (!response.ok) throw new Error("GOOGLE_OAUTH_TOKEN_HTTP_ERROR");
    const result = await boundedJson(response, 64 * 1024);
    if (typeof result.refresh_token !== "string" || typeof result.id_token !== "string" || typeof result.scope !== "string") throw new Error("GOOGLE_OAUTH_TOKEN_RESPONSE_INVALID");
    const identity = await validateGoogleIdToken(result.id_token, this.options.clientId, this.options.fetch ?? fetch);
    return Object.freeze({ ...identity, grantedScopes: Object.freeze(result.scope.split(/\s+/u).filter(Boolean).map(canonicalGoogleOAuthScope)), refreshToken: result.refresh_token, clientId: this.options.clientId, ...(this.options.clientSecret === undefined ? {} : { clientSecret: this.options.clientSecret }) });
  }
}
