...
describe("worker-side account binding discovery", () => {
  it("returns only the exact grant-bound actor/workspace binding", async () => {
    ...
    await new FileAccountBindingStore(path).upsert({ subjectId, workspaceId, accountId: defineAccountId("acct_0123456789abcdef"), credentialHandle: defineCredentialHandle("cred_0123456789abcdef"), credentialGeneration: 1, allowedActions: ["calendar.events.list"], enabled: true });
    ...
    const worker = new IsolatedCredentialWorker({ verifier: new CredentialGrantVerifier({ secret, issuer: "gateway", audience: "worker", now: () => 101 }), credentials: {} as never, replay: new MemoryGrantReplayStore(() => 101), audit: new MemoryAuditSink() }, [createAccountBindingResolveOperation(path)]);
