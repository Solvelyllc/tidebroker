...
export const GOOGLE_WRITE_TOOL_NAMES = new Set(["google_calendar_event_create", "google_calendar_event_update", "google_calendar_event_delete", "google_project_services_enable", "google_gmail_message_send"]);
...
function description(toolName: string, params: Record<string, unknown>): string {
  if (toolName === "google_project_services_enable") return `Enable exactly these APIs in the existing Google Cloud project: ${Array.isArray(params.services) ? params.services.filter((item): item is string => typeof item === "string").slice(0, 20).join(", ") : "(invalid list)"}.`;
  if (toolName === "google_gmail_message_send") return `Send email to ${Array.isArray(params.to) ? params.to.filter((item): item is string => typeof item === "string").slice(0, 10).join(", ") : "(invalid recipients)"} with subject “${safe(params.subject, "(no subject)")}” and exactly the proposed body.`;
...
export function requireGoogleWriteApproval(event: { toolName: string; toolCallId?: string; params: Record<string, unknown> }, context: { requester?: { channel?: string; accountId?: string; senderId?: string } }): unknown {
  ...
  const projectAdmin = event.toolName === "google_project_services_enable"; const emailSend = event.toolName === "google_gmail_message_send";
  return { requireApproval: { title: projectAdmin ? "Approve Google project change" : emailSend ? "Approve email send" : "Approve Google Calendar change", description: description(event.toolName, event.params), severity: projectAdmin || emailSend || event.toolName === "google_calendar_event_delete" ? "critical" : "warning", timeoutMs: 5 * 60 * 1000, timeoutReason: "Google change was not approved.", allowedDecisions: ["allow-once", "deny"], pluginId: "tidebroker", onResolution: (decision: string) => { if (decision === "allow-once") tickets.set(ticketKey, { digest, actor: approvalActor, expiresAt: Date.now() + 2 * 60 * 1000 }); else tickets.delete(ticketKey); } } };
}
