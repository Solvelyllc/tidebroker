...
export async function recordRealProviderEvidence(resultsPath, outputPath, sourceCommit) {
  ...
  const info = await lstat(resultsPath).catch(fail);
  if (!info || !info.isFile() || info.isSymbolicLink() || (info.mode & 0o077) !== 0 || typeof process.getuid === "function" && info.uid !== process.getuid()) fail();
  let input; try { input = JSON.parse(await readFile(resultsPath, "utf8")); } catch { fail(); }
